...
const owner=readFileSync("app/components/WeedOwnerPage.tsx","utf8");const route=readFileSync("app/weed-dispensary-brampton/page.tsx","utf8");const home=readFileSync("app/HomePage.tsx","utf8");
test("generic city URL is demoted to the Torbram corridor owner",()=>{assert.match(route,/index: false/);assert.match(route,/follow: true/);assert.match(route,/weed-dispensary-torbram/);assert.doesNotMatch(route,/canonical: "https:\/\/www\.planets59\.com\/weed-dispensary-brampton"/);});
