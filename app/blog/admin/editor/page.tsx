...
function BlogEditorInner() {
  ...
  const [author, setAuthor] = useState("PLANETS 59 Team");
  ...
  useEffect(() => {
    ...
    fetch(`${APPS_SCRIPT_URL}?action=blog&store=PL501&admin=1`)
      .then((r) => r.json())
      .then((data) => {
        ...
        if (post) {
          ...
          setAuthor(post.author || "PLANETS 59 Team");
  ...
  async function handleSave() {
    ...
    const payload = editId
      ...
      : { action: "create", title, content, author, published, store: "PL501" };
